...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-loanmig-')), 'unused.db');
...
function seedPreV101() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-loanmig-')), 'db.sqlite'));
  db.exec(`
