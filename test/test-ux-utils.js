...
test('date inputs: accept slash, dot, and hyphen separators for DMY dates', () => {
  localStorage.setItem('aashiyana-date-format', 'dmy');
...
test('date inputs: accept hyphen separators for YMD dates', () => {
  localStorage.setItem('aashiyana-date-format', 'ymd');
  assert.equal(parseDateInput('2026-5-6'), '2026-05-06');
  assert.equal(parseDateInput('2026/05/06'), '2026-05-06');
  assert.equal(parseDateInput('2026.05.06'), '2026-05-06');
});

test('task + recurrence date fields use the shared aashiyana-datepicker', () => {
  ...
  assert.match(tasksSource, /<aashiyana-datepicker type="date"[\s\S]*?name="start_date"/);
  assert.match(tasksSource, /<aashiyana-datepicker type="date"[\s\S]*?name="due_date"/);
  assert.match(tasksSource, /<aashiyana-datepicker type="time"[\s\S]*?name="due_time"/);
  assert.match(rruleSource, /<aashiyana-datepicker type="date"[\s\S]*?id="\$\{prefix\}-rrule-until"/);
...
test('stagger: tut nichts bei prefers-reduced-motion', () => {
  ...
  const els = [{ style: {} }];
  stagger(els);
  assert.equal(els[0].style.opacity, undefined);
...
test('scheduleUndoableDelete: Undo verhindert den Server-Delete', async () => {
  ...
  global.window.aashiyana = { showToast: (_msg, _type, _duration, undoFn) => { capturedUndo = undoFn; } };
...
test('parseTimeInput: bare hour (24 h) expands to HH:00', () => {
  localStorage.setItem('aashiyana-time-format', '24h');
...
test('formatTimeInput: bare hour (12 h) formats with AM/PM', () => {
  localStorage.setItem('aashiyana-time-format', '12h');
...
test('parseDateInput: 8 raw digits (MDY)', () => {
  localStorage.setItem('aashiyana-date-format', 'mdy');
