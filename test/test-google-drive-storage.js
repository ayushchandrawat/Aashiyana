...
test('callback validates the account, creates and reuses Aashiyana/Documents, and never selects Drive', async () => {
  const first = await driveStorage.handleCallback('code');
  assert.equal(first.connected, true);
  assert.equal(first.account_email, 'a@example.test');
  assert.equal(first.folder_name, 'Aashiyana/Documents');
