...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-pantrymig-')), 'unused.db');
const { MIGRATIONS } = await import('../server/db.js');

const V162 = MIGRATIONS.find((m) => m.version === 162);

function seedPreV162() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-pantrymig-')), 'db.sqlite'));
  db.exec(`
