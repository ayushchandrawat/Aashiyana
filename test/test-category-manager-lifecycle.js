...
globalThis.window = { aashiyana: { showToast() {} } };
globalThis.CSS = { escape: String };

let CategoryManager;
globalThis.customElements = {
  define(name, constructor) {
    if (name === 'aashiyana-category-manager') CategoryManager = constructor;
  },
};
...
test('a stale 409 reports the error without reopening rename in a newer modal context', async () => {
  const previousPut = api.put;
  const previousShowToast = globalThis.window.aashiyana.showToast;
  const defaults = [];
  const toasts = [];
  ...
  globalThis.window.aashiyana.showToast = (message, tone) => { toasts.push({ message, tone }); };
  api.put = async () => {
    attempts += 1;
  ...
  try {
    ...
    api.put = previousPut;
    globalThis.window.aashiyana.showToast = previousShowToast;
