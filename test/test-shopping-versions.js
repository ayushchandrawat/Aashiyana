...
const NOTES_TOKEN = 'aashiyana_test_notes_only_token';
db.prepare(`
  INSERT INTO api_tokens (name, token_hash, token_prefix, created_by, scopes)
  VALUES ('notes-only', ?, 'aashiyana_test', 1, ?)
...
test('v196: bestehende Listen bekommen beim Update ihre Zeile mit 0', () => {
  ...
  const old = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-versions-')), 'db.sqlite'));
  old.exec(`
