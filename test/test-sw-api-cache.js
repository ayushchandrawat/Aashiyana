...
async function apiCacheName(env) {
  return (await env.caches.keys()).find((n) => n.startsWith('aashiyana-api-'));
}
...
test('activate entfernt alte Vorversions- und Legacy-oikos-Caches, behält aktuelle Versions-Caches', async () => {
  ...
  await env.caches.open('aashiyana-api-0.0.1');
  await env.caches.open('oikos-shell-0.0.1');
  const currentRelease = `${pkg.version}-${TEST_BUILD_REVISION}`;
  await env.caches.open(`aashiyana-shell-${currentRelease}`);  // aktuell → behalten
  await env.caches.open(`aashiyana-api-${currentRelease}`);    // aktuell → behalten

  await dispatchActivate(env);

  assert.equal(await env.caches.has('aashiyana-api-0.0.1'), false, 'alter API-Cache muss weg sein');
  assert.equal(await env.caches.has('oikos-shell-0.0.1'), false, 'Legacy-oikos-Cache muss weg sein');
  assert.equal(await env.caches.has(`aashiyana-shell-${currentRelease}`), true, 'aktueller Shell-Cache bleibt');
  assert.equal(await env.caches.has(`aashiyana-api-${currentRelease}`), true, 'aktueller API-Cache bleibt');
});
