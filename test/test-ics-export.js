...
test('buildFeed: Ganztags-Override nutzt Master-UID und RECURRENCE-ID ohne gepaarte EXDATE', () => {
  ...
  assert(master.includes(`UID:event-${masterId}@aashiyana`), 'Master-UID falsch: ' + master);
  ...
  assert(child.includes(`UID:event-${masterId}@aashiyana`), 'Replacement muss Master-UID nutzen: ' + child);
  assert(!child.includes(`UID:event-${childId}@aashiyana`), 'Replacement darf keine Child-UID nutzen: ' + child);
...
test('buildFeed: verschobener Ersatz behält seinen abgelaufenen Master im Feed', () => {
  ...
  assert(master.includes(`UID:event-${masterId}@aashiyana`), 'Master-UID fehlt: ' + master);
  ...
  assert(child?.includes(`UID:event-${masterId}@aashiyana`), 'Replacement muss die Master-UID nutzen: ' + child);
...
test('buildFeed: unreachable linked child degrades standalone and keeps the master EXDATE', () => {
  ...
  assert(child?.includes(`UID:event-${childId}@aashiyana`),
...
d2.prepare(`INSERT INTO event_assignments (event_id,user_id) VALUES (?,?)`).run(soloId, u3);
