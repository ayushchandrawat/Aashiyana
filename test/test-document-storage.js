...
function configureFakeDriveStorage() {
  ...
  for (const [key, value] of Object.entries({
    ...
    folder_name: 'Aashiyana/Documents',
...
test('local folder remains the effective override when Google Drive is selected', () => {
  ...
  process.env.DOCUMENT_STORAGE_LOCAL_PATH = '/tmp/aashiyana-drive-override';
  assert.equal(storage.getSelectedUploadBackend(), 'google_drive');
...
test('getConfig defaults and normalizes safe paths while rejecting invalid config', async () => {
  ...
  assert.equal(storage.getConfig().basePath, 'aashiyana-documents');
...
function withLocalStorage(fn) {
  const dir = mkdtempSync(nodePath.join(tmpdir(), 'aashiyana-docs-'));
...
test('document routes: local folder upload lands on disk and status reports local_folder', async (t) => {
  const dir = mkdtempSync(nodePath.join(tmpdir(), 'aashiyana-docs-route-'));
  const previous = Object.fromEntries(LOCAL_ENV_KEYS.map((k) => [k, process.env[k]]));
  process.env.DOCUMENT_STORAGE_LOCAL_ENABLED = 'true';
  process.env.DOCUMENT_STORAGE_LOCAL_PATH = dir;
  const userId = createRouteUser();
...
test('mount check: meldet einen fehlenden Ordner als fehlenden Mount (#751)', async () => {
  ...
  const missing = nodePath.join(tmpdir(), `aashiyana-nomount-${randomUUID()}`);
  process.env.DOCUMENT_STORAGE_LOCAL_ENABLED = '1';
...
test('mount check: schweigt bei einem vorhandenen, beschreibbaren Ordner (#751)', async () => {
  const dir = mkdtempSync(nodePath.join(tmpdir(), 'aashiyana-mounted-'));
  process.env.DOCUMENT_STORAGE_LOCAL_ENABLED = '1';
  process.env.DOCUMENT_STORAGE_LOCAL_PATH = dir;
  try {
    const log = collectingLogger();
...
test('mount check: meldet eine Datei am Ziel statt eines Ordners (#751)', async () => {
  const dir = mkdtempSync(nodePath.join(tmpdir(), 'aashiyana-notdir-'));
