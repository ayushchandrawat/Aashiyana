...
test('buildWasteFeed: emits a stable UID/all-day VEVENT for a seeded occurrence, honors the type selection', async () => {
  ...
  assert.match(unfiltered, new RegExp(`UID:waste-pickup-${shown.id}-${dateKey}@aashiyana`));
  assert.match(unfiltered, new RegExp(`UID:waste-pickup-${hidden.id}-${dateKey}@aashiyana`));

  setFeedTypeIds(db, ADMIN, [shown.id]);
  const filtered = buildWasteFeed(db, ADMIN);
  assert.match(filtered, new RegExp(`UID:waste-pickup-${shown.id}-${dateKey}@aashiyana`));
  assert.doesNotMatch(filtered, new RegExp(`UID:waste-pickup-${hidden.id}-${dateKey}@aashiyana`));
