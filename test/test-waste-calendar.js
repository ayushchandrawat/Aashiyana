...
function withWasteViewState(extra, windowExtra, fn) {
  ...
  try {
    ...
    globalThis.window = {
      ...
      aashiyana: { navigate: (path) => navigated.push(path), isModuleDisabled: () => false },
      ...windowExtra,
    };
...
test('wasteEnabled(): false, wenn das Modul abgeschaltet ist', () => {
  withWasteViewState({}, { aashiyana: { isModuleDisabled: () => true, navigate: () => {} } }, () => {
...
test('activeFilterCount(): die AUS-Vorgabe der Waste-Ebene zaehlt nie - nur eine gewaehlte Typ-Teilmenge (Round-3-Blocker)', () => {
  ...
  withWasteViewState(
    ...
    { aashiyana: { isModuleDisabled: () => true, navigate: () => {} } },
    () => {
...
test('renderMonthView: Klick (Desktop) auf den Waste-Chip navigiert direkt und fuehrt NICHT in switchToDayView', () => {
  withWasteViewState({ wasteOccurrences: [occurrence()] }, {}, (navigated) => {
    ...
    assert(navigated.length === 1 && navigated[0] === '/waste?type=1&date=2026-09-10',
      `der Klick muss window.aashiyana.navigate('/waste?type=1&date=2026-09-10') ausloesen: ${JSON.stringify(navigated)}`);
