...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-calmig-')), 'unused.db');
...
function buildPreOutboundDatabase() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-calmig-')), 'db.sqlite'));
