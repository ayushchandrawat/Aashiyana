...
test('sanitizeEnv akzeptiert alle Dokument-WebDAV-Schemawerte', () => {
  const values = {
    ...
    DOCUMENT_STORAGE_WEBDAV_PATH: '/aashiyana/documents',
  };
...
test('renderEnvFile erzeugt KEY=value-Zeilen mit Header und Schluss-Newline', () => {
  ...
  assert.match(out, /^# Generated by Aashiyana Installer\n/);
...
test('encodeEnvValue lässt einfache Werte unverändert (bare)', () => {
  for (const v of ['abc123', 'Europe/Berlin', 'https://dav.example.test', '/aashiyana/documents', 'true', '587']) {
...
test('POST /api/save-env schreibt alle Dokument-WebDAV-Werte', async () => {
  ...
  try {
    await withServer(dir, async base => {
      ...
      assert.match(content, /^DOCUMENT_STORAGE_WEBDAV_PATH=\/aashiyana\/documents$/m);
    });
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test('POST /api/save-env writes Google Drive OAuth values safely', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'aashiyana-drive-env-'));
  try {
    await withServer(dir, async (base) => {
