...
test('Definiert das Custom Element aashiyana-datepicker', () => {
  assert(/customElements\.define\(\s*'aashiyana-datepicker'/.test(comp), 'Tag-Name muss aashiyana-datepicker sein');
});
test('Registrierung ist idempotent (guard gegen Doppel-Define)', () => {
  assert(/if\s*\(\s*!customElements\.get\(\s*'aashiyana-datepicker'\s*\)\s*\)/.test(comp), 'Define muss geguardet sein');
});
