...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-vismig-')), 'unused.db');
...
function seedPreV156() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-vismig-')), 'db.sqlite'));
  db.exec(`
