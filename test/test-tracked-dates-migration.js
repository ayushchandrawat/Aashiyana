...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-trackeddatesmig-')), 'unused.db');
const { MIGRATIONS } = await import('../server/db.js');

const V141 = MIGRATIONS.find((m) => m.version === 141);

function seedPreV141() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-trackeddatesmig-')), 'db.sqlite'));
  db.exec(`
