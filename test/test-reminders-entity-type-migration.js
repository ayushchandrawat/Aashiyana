...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-remindermig-')), 'unused.db');
...
function seedPreV140() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-remindermig-')), 'db.sqlite'));
  db.exec(`
