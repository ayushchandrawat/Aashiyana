...
test('GET / liefert die dokumentierten Defaults', async () => {
  ...
  assert.equal(body.data.app_name, 'Aashiyana');
...
test('PUT app_name: gültig -> persist, leer -> Rückfall auf Default', async () => {
  ...
  assert.equal((await put({ app_name: '   ' })).body.data.app_name, 'Aashiyana');
  assert.equal((await get()).body.data.app_name, 'Aashiyana');
});
...
test('PUT calendar_default_reminders: Validierung + dedup/sort persist', async () => {
  ...
  assert.equal((await put({ calendar_default_reminders: [7] })).status, 400);
