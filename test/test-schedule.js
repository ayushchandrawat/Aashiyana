...
test('schedule statistics tab uses the computed entries API and includes overnight and 24-hour durations', () => {
  ...
  assert.match(schedulePage, /aashiyana-datepicker required name="from" type="date"/);
...
test('an inverted or incomplete custom statistics range never fetches and never sets the generic error state (S-06)', () => {
  ...
  assert.match(schedulePage, /formField\(t\('schedule\.rangeFrom'\), '<aashiyana-datepicker required name="from"/, 'the custom-range field must use rangeFrom/rangeTo, not the pattern-vocabulary validFrom/validUntil');
  assert.match(schedulePage, /formField\(t\('schedule\.rangeTo'\), '<aashiyana-datepicker required name="to"/);
});
...
test('switching tabs navigates through the router (URL + history entry), and the router registers one exact route per tab (S-10)', () => {
  ...
  assert.match(guardedFn, /window\.aashiyana\?\.navigate\(scheduleRouteForView\(id\)\)/, 'a tab click must go through navigate(), not a bare activateView() call, or the URL never updates');
