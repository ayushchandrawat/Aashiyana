...
describe('WebDAV Backup — service module', async () => {
  ...
  describe('uploadBackup()', async () => {
    ...
    it('should upload and list new aashiyana-prefixed backups (post-rebrand naming)', async () => {
      const fp = await createTempBackup('aashiyana-backup-2099-02-01T00-00-00-000Z.db');
      await webdav.uploadBackup(fp);
      assert.ok(
        mockCtx.files.has(`/oikos/backups/${path.basename(fp)}`),
        'aashiyana-prefixed file should be uploaded'
      );
      const listed = await webdav.getRemoteFiles();
      assert.ok(
        listed.some((f) => f.filename.startsWith('aashiyana-backup-')),
        'aashiyana-prefixed file should be recognised by the file listing'
      );
    });
  ...
  describe('rotation on an Apache mod_dav server (#853)', async () => {
    ...
    it('should keep the newest backups and delete the oldest', async () => {
      const names = [
        'aashiyana-backup-2099-03-01T00-00-00-000Z.db',
        'aashiyana-backup-2099-03-02T00-00-00-000Z.db',
        'aashiyana-backup-2099-03-03T00-00-00-000Z.db',
        'aashiyana-backup-2099-03-04T00-00-00-000Z.db',
        'aashiyana-backup-2099-03-05T00-00-00-000Z.db',
      ];
      for (const name of names) {
    ...
    it('should never delete the file it just uploaded', async () => {
      const fresh = 'aashiyana-backup-2099-03-06T00-00-00-000Z.db';
  ...
  describe('rotation on a server that answers with absolute hrefs', async () => {
    ...
    it('should still delete the oldest file', async () => {
      const names = [
        'aashiyana-backup-2099-04-01T00-00-00-000Z.db',
        'aashiyana-backup-2099-04-02T00-00-00-000Z.db',
        'aashiyana-backup-2099-04-03T00-00-00-000Z.db',
        'aashiyana-backup-2099-04-04T00-00-00-000Z.db',
      ];
      for (const name of names) {
        await webdav.uploadBackup(await createTempBackup(name));
      }
...
describe('env-Vorrang', () => {
  it('behandelt leere env-Variablen als nicht gesetzt', async () => {
    ...
    try {
      ...
      assert.equal(cfg.remotePath, '/aashiyana/backups/', 'der Default-Pfad muss greifen');
