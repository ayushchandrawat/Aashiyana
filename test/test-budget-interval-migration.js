...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-intervalmig-')), 'unused.db');
const { MIGRATIONS } = await import('../server/db.js');

const V128 = MIGRATIONS.find((m) => m.version === 128);

function seedPreV128() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-intervalmig-')), 'db.sqlite'));
  db.exec(`
