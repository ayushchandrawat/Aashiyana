...
test('Definiert das Custom Element aashiyana-category-manager', () => {
  assert(/customElements\.define\(\s*'aashiyana-category-manager'/.test(comp), 'Tag-Name muss aashiyana-category-manager sein');
});
...
test('Budget importiert die generische Komponente', () => {
  ...
  assert(/aashiyana-category-manager/.test(budgetPage), 'budget.js muss das Element verwenden');
});
test('Notizen importieren die generische Komponente auch beim direkten Seitenaufruf', () => {
  ...
  assert(/aashiyana-category-manager/.test(notesPage), 'notes.js muss das Element verwenden');
});
