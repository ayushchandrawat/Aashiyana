...
test('Tagesraster: der Now-Punkt sitzt in der Stundenspalte, wo nie ein Termin steht', () => {
  ...
  assert(/inset-inline-start|left/.test(rule.body) && /--cal-gutter-width/.test(rule.body),
    'der Punkt muss seine Position aus
...
test('#1064: "Nicht zugewiesen" ist ein Eintrag der Personenachse', () => {
  ...
  mitFilterzustand({ ...basis, people: new Set([1]) }, () => {
    assert(tagesIds() === '1',
      `ein Filter ohne den Eintrag - etwa einer von vor
  });
});
...
test('#1064: beide Filter kommen gegen den Speicher geprueft zurueck', () => {
  ...
  mitSpeicher({ 'aashiyana:calendar:people': JSON.stringify([1, UNASSIGNED, 99]) }, () => {
  ...
  mitSpeicher({ 'aashiyana:calendar:sources-hidden:1': '{kaputt' }, () => {
...
test('#1064: die ausgeblendeten Quellen gehoeren dem Nutzer, nicht dem Geraet', () => {
  ...
  mitSpeicher({ 'aashiyana:calendar:sources-hidden:1': JSON.stringify([{ key: 'sub:7', name: 'Privat', color: null }]) }, () => {
  ...
  mitSpeicher({ 'aashiyana:calendar:sources-hidden': JSON.stringify([{ key: 'sub:7', name: 'Alt', color: null }]) }, () => {
  ...
  mitSpeicher({}, (daten) => {
    ...
    assert([...daten.keys()].join(',') === 'aashiyana:calendar:sources-hidden:3', `geschrieben unter dem Nutzer, war ${[...daten.keys()]}`);
...
test('#1064: eine ausgeblendete Quelle zaehlt am Filterknopf als ein Filter', () => {
  ...
  // scheduleEnabled()/wasteEnabled() fragen window.aashiyana - ohne Modul-Registry gelten Schichtplan
