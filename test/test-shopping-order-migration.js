...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-sortmig-')), 'unused.db');
const { MIGRATIONS } = await import('../server/db.js');

const V133 = MIGRATIONS.find((m) => m.version === 133);

function seedPreV133() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-sortmig-')), 'db.sqlite'));
  db.exec(`
