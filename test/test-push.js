...
test('VAPID_SUBJECT wins over every other source', () => {
  ...
  const subject = resolvedSubject(db, {
    ...
    BASE_URL: 'https://aashiyana.example.net',
  });
...
test('VAPID subject uses the configured sender address before BASE_URL', () => {
  const db = makeDb();
  db.prepare("INSERT INTO sync_config (key,value) VALUES ('email_from_address','box@mail.example')").run();
  assert.equal(resolvedSubject(db, { BASE_URL: 'https://aashiyana.example.net' }), 'mailto:box@mail.example');
});

test('VAPID subject falls back to the BASE_URL origin', () => {
  const subject = resolvedSubject(makeDb(), { BASE_URL: 'https://aashiyana.example.net/app/' });
  assert.equal(subject, 'https://aashiyana.example.net');
});
...
test('VAPID subject skips an unusable sender address and keeps BASE_URL', () => {
  ...
  assert.equal(resolvedSubject(db, { BASE_URL: 'https://aashiyana.example.net' }), 'https://aashiyana.example.net');
});
