...
process.env.DB_PATH = join(mkdtempSync(join(tmpdir(), 'aashiyana-cyclesymptomsmig-')), 'unused.db');
const { MIGRATIONS } = await import('../server/db.js');

const V178 = MIGRATIONS.find((m) => m.version === 178);

function seedPreV178() {
  const db = new Database(join(mkdtempSync(join(tmpdir(), 'aashiyana-cyclesymptomsmig-')), 'db.sqlite'));
  db.exec(`
