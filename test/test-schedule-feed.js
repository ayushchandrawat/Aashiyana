...
test('buildScheduleFeed enthält nur die eigenen Einträge des Feed-Besitzers', () => {
  ...
  assert.match(ics, new RegExp(`UID:schedule-entry-${alice}-${today}@aashiyana`));
...
test('ein Extra am selben Tag wie der primaere Eintrag bekommt eine eigene UID, beide VEVENTs bleiben erhalten', () => {
  ...
  assert.match(ics, new RegExp(`UID:schedule-entry-${alice}-${today}@aashiyana`), 'the primary entry keeps its plain UID');
  assert.match(ics, new RegExp(`UID:schedule-entry-${alice}-${today}-extra-${extraId}@aashiyana`), 'the extra carries its own id so the UID never collides with the primary entry');
...
test('zwei Klassen am selben Tag (ein Musterzyklus-Tag) bekommen je eine eigene UID', () => {
  ...
  assert.match(ics, new RegExp(`UID:schedule-entry-${alice}-${today}-pattern-${mathDayId}@aashiyana`));
  assert.match(ics, new RegExp(`UID:schedule-entry-${alice}-${today}-pattern-${bioDayId}@aashiyana`));
...
test('buildScheduleFeed liefert ein valides VCALENDAR-Gerüst auch ohne Einträge', () => {
  ...
  assert.match(ics, /X-WR-CALNAME:Aashiyana Schedule/);
});
