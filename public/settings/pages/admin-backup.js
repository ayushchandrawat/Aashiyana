...
function buildCliContent() {
  ...
  wrap.insertAdjacentHTML('beforeend', `
    ...
    <pre class="settings-code-block"><code>SERVICE=aashiyana
BACKUP="$PWD/aashiyana-backup.db"
...
docker compose up -d "$SERVICE"</code></pre>
    ...
    <pre class="settings-code-block"><code>docker compose exec aashiyana node -e "import('./server/db.js').then(async db =&gt; { await db.backupToFile('/data/aashiyana-backup.db'); process.exit(0); })"
docker cp aashiyana:/data/aashiyana-backup.db ./aashiyana-backup.db</code></pre>
  `);
  return wrap;
}
    ...
    if (triggerBtn) {
      triggerBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('settings.backupSchedulerTriggeredToast'), 'success');
  ...
  if (triggerBtn) {
    triggerBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.aashiyana?.showToast(t('settings.backupWebdavTriggeredToast'), 'success');
...
async function loadWebdavConfig(container) {
  ...
  try {
    ...
    setVal('webdav-path', d.remotePath ?? '/aashiyana/backups/');
...
function bindWebdavBackupEvents(container) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      remotePath: form.querySelector('#webdav-path')?.value?.trim() || '/aashiyana/backups/',
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.backupWebdavSaved'), 'success');
      loadWebdavConfig(container);
    } catch (err) {
      window.aashiyana?.showToast(err.message ?? t('common.errorGeneric'), 'danger');
    } finally {
...
function bindRestoreEvents(container) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.backupRestoredToast'), 'success');
