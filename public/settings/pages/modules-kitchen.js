...
function bindEvents(container) {
  ...
  mealToggles?.addEventListener('change', async () => {
    ...
    if (checkedMealTypes.length === 0) {
      ...
      window.aashiyana?.showToast(t('settings.mealTypesMinOne'), 'danger');
      return;
    }
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.mealTypesSaved'), 'success');
    } catch (error) {
  ...
  nameFields?.addEventListener('change', async (event) => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.mealTypeNamesSaved'), 'success');
    } catch (error) {
