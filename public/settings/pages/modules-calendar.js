...
function bindWeekStart(container, preferences) {
  ...
  toggle.addEventListener('click', async (event) => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.weekStartSaved'), 'success');
    } catch (error) {
      current = previous;
...
async function bindEvents(container, preferences) {
  ...
  container.querySelector('#calendar-personal-link')?.addEventListener('click', (event) => {
    ...
    window.aashiyana.navigate(PERSONAL_CALENDAR_PATH);
  });
  ...
  durationSelect?.addEventListener('change', async () => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.calendarDurationSaved'), 'success');
    } catch (error) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.holidaySaved'), 'success');
    } catch (error) {
      errorElement.textContent = error.message || t('common.errorGeneric');
      errorElement.hidden = false;
    }
  ...
  syncButton.addEventListener('click', async () => {
    ...
    if (!currentPreferenceData.holiday_country) {
      window.aashiyana?.showToast(t('settings.holidayCountryRequired'), 'warning');
      return;
    }
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.holidaySynced'), 'success');
    } catch (error) {
      window.aashiyana?.showToast(error.message || t('settings.holidaySyncError'), 'danger');
    } finally {
      updateSyncState();
    }
  });
