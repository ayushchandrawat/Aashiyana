      ...
      <div class="settings-card settings-card--hidden" id="add-member-form-card">
        ...
        <form id="add-member-form" class="settings-form">
          ...
          <div class="form-group">
            ...
            <aashiyana-datepicker type="date" id="new-member-birth-date"></aashiyana-datepicker>
...
function bindInviteEvents(container, initialInvites) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      outputValue.focus();
      outputValue.select();
      window.aashiyana?.showToast(t('settings.invites.created'), 'success');
    } catch (err) {
  ...
  list.addEventListener('click', async (event) => {
    ...
    try {
      ...
      renderInviteList(container, invites);
      refocusAfterRender();
      window.aashiyana?.showToast(t('settings.invites.revoked'), 'default');
    } catch (err) {
  ...
  container.querySelectorAll('[data-delete-user]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.aashiyana?.showToast(t('settings.memberDeletedToast', { name }), 'default');
      } catch (err) {
    ...
    content: `
      <form id="edit-member-form" class="settings-form">
        ...
        <div class="form-group">
          ...
          <aashiyana-datepicker type="date" id="edit-member-birth-date" value="${esc(member.birth_date || '')}"></aashiyana-datepicker>
          <p class="form-hint">${t('settings.memberContactBirthdayHint')}</p>
        </div>
        ${ssoAvailable ? `
        ${toggleRowHtml({
    ...
    onSave(panel) {
      ...
      panel.querySelector('#edit-member-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('settings.memberUpdatedToast', { name: res.user.display_name }), 'success');
  ...
  if (addMemberForm) {
    ...
    addMemberForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        container.querySelector('#add-member-form-card').classList.add('settings-card--hidden');
        container.querySelector('#add-member-btn').hidden = false;
        window.aashiyana?.showToast(t('settings.memberAddedToast', { name: res.user.display_name }), 'success');
        bindDeleteButtons(container);
        bindEditButtons(container, currentUser, users);
      } catch (err) {
        showError(errorEl, err.message);
