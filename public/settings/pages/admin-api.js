      ...
      <div class="settings-card">
        ...
        <form id="api-token-form" class="settings-form" autocomplete="off">
          ...
          <div class="form-group">
            ...
            <aashiyana-datepicker type="datetime" id="api-token-expires"></aashiyana-datepicker>
...
function bindEvents(container, initialTokens, users, currentUserId) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      outputValue.focus();
      outputValue.select();
      window.aashiyana?.showToast(t('settings.apiTokenCreatedToast'), 'success');
    } catch (err) {
  ...
  container.querySelector('#api-token-copy')?.addEventListener('click', async () => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('settings.apiTokenCopied'), 'success');
    } catch (err) {
  ...
  list.addEventListener('click', async (event) => {
    ...
    try {
      ...
      renderApiTokenList(container, tokens);
      refocusAfterRender();
      window.aashiyana?.showToast(t('settings.apiTokenRevokedToast'), 'default');
    } catch (err) {
