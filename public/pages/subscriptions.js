...
async function reload(options) {
  try {
    ...
    window.aashiyana?.showToast(err.data?.error || t('subscriptions.loadError'), 'danger');
  }
}
      ...
      <section class="subscription-form__section">
        ...
        <div class="form-grid-2">
          <div class="form-group">
            ...
            <aashiyana-datepicker id="subscription-next-date" type="date"
                   value="${esc(subscription?.next_payment_date || todayKey())}"></aashiyana-datepicker>
          </div>
          <div class="form-group">
        ...
        <div class="form-grid-2">
          <div class="form-group" id="subscription-end-date-field" ${(subscription?.end_type === 'on_date') ? '' : 'hidden'}>
            ...
            <aashiyana-datepicker id="subscription-end-date" type="date"
                   value="${esc(subscription?.end_date || '')}"></aashiyana-datepicker>
          </div>
    ...
    onSave(panel) {
      ...
      panel.querySelector('#subscription-find-logo').addEventListener('click', async () => {
        openLogoPickerModal(panel, subscription?.website_url || panel.querySelector('#subscription-name').value.trim(), (logoData) => {
          ...
          showLogo(searchedLogoData);
          window.aashiyana?.showToast(t('subscriptions.logoFound'), 'success');
        });
      });
...
async function saveSubscription(panel, existing, searchedLogoData = null) {
  ...
  try {
    ...
    await reload();
    refocusAfterRender();
    window.aashiyana?.showToast(t(existing ? 'subscriptions.savedToast' : 'subscriptions.addedToast'), 'success');
  } catch (err) {
    window.aashiyana?.showToast(err.data?.error || err.message || t('common.unknownError'), 'danger');
  } finally {
...
async function renewSubscription(subscription) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t(completed ? 'subscriptions.completedToast' : 'subscriptions.renewedToast'), 'success');
  } catch (err) {
...
async function deleteSubscription(subscription) {
  ...
  try {
    ...
    await reload();
    refocusAfterRender();
    window.aashiyana?.showToast(t('subscriptions.deletedToast'), 'success');
  } catch (err) {
    window.aashiyana?.showToast(err.data?.error || t('common.unknownError'), 'danger');
  }
}
    ...
    onSave(panel) {
      ...
      panel.querySelector('#subscriptions-settings-form').addEventListener('submit', async (event) => {
        ...
        try {
          ...
          refocusAfterRender();
          window.aashiyana?.showToast(t('subscriptions.settingsSaved'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error || t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelectorAll('[data-act="save"]').forEach((button) => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            refocusAfterRender();
            openMetadataModal();
            window.aashiyana?.showToast(t('subscriptions.metaSavedToast'), 'success');
          } catch (err) {
            window.aashiyana?.showToast(err.data?.error || err.message || t('common.unknownError'), 'danger');
          }
        });
      ...
      panel.querySelectorAll('[data-act="delete"]').forEach((button) => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            await reload();
            refocusAfterRender();
            window.aashiyana?.showToast(t('subscriptions.metaDeletedToast'), 'success');
          } catch (err) {
            window.aashiyana?.showToast(err.data?.error || err.message || t('common.unknownError'), 'danger');
          }
          openMetadataModal();
