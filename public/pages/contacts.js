...
export async function render(container, { user }) {
  ...
  _container.querySelector('#contacts-import-input').addEventListener('change', async (e) => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('contacts.importError', { error: err.message }), 'danger');
      return;
    }
    ...
    if (named.length === 0) { window.aashiyana?.showToast(t('contacts.vcardNoName'), 'warning'); return; }
...
function buildContactForm({ mode, contact = null }) {
  ...
  const advancedFieldsHtml = `
    ...
    <div class="form-group">
      ...
      <aashiyana-datepicker id="cm-birthday" type="date" value="${v('birthday')}"></aashiyana-datepicker>
  ...
  return {
    ...
    wire(panel) {
      ...
      panel.querySelector('#cm-save').addEventListener('click', async () => {
        ...
        if (!name) {
          window.aashiyana?.showToast(t('contacts.nameRequiredHint'), 'danger');
        ...
        try {
          ...
          closeModal({ force: true });
          renderList();
          window.aashiyana?.showToast(mode === 'create' ? t('contacts.savedToast') : t('contacts.updatedToast'), 'success');
        } catch (err) {
...
function openBirthdayImport() {
  try { sessionStorage.setItem('aashiyana:birthdays:autoImport', '1'); } catch { /* egal */ }
  window.aashiyana?.navigate('/birthdays');
}
...
async function importParsedContacts(list) {
  ...
  if (imported === 0) {
    ...
    const reason = window.aashiyana?.friendlyError?.(lastError) || lastError?.message || '';
  ...
  window.aashiyana?.showToast(message, type, action ? 6000 : 3000, action);
}
