...
async function toggleSession(container, workerId) {
  ...
  if (!state.workers.length) {
    window.aashiyana?.showToast(t('housekeeping.checkInDisabled'), 'warning');
    return;
  }
  if (!worker) return;
  try {
    if (current) {
      ...
      window.aashiyana?.showToast(t('housekeeping.checkedOutToast'), 'success');
    } else {
      ...
      window.aashiyana?.showToast(t('housekeeping.checkedInToast'), 'success');
    }
    await loadData();
...
async function createTask(payload, content) {
  try {
    ...
    window.aashiyana?.showToast(t('housekeeping.taskCreatedToast'), 'success');
    await loadData();
    renderTasks(content);
  } catch (err) {
    window.aashiyana?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-complete-task]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      try {
        ...
        window.aashiyana?.showToast(t('housekeeping.taskDoneToast'), 'success');
        await loadData();
        renderTasks(content);
      } catch (err) {
        window.aashiyana?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-undo-task]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      try {
        ...
        window.aashiyana?.showToast(t('housekeeping.taskUndoneToast'), 'success');
        await loadData();
        renderTasks(content);
      } catch (err) {
        window.aashiyana?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-delete-task]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.aashiyana?.showToast(t('housekeeping.taskDeletedToast'), 'success');
        await loadData();
        renderTasks(content);
        refocusAfterRender();
      } catch (err) {
...
async function payVisit(visit, onPaid) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('housekeeping.visitPaidToast'), 'success');
    await onPaid();
    refocusAfterRender();
  } catch (err) {
    window.aashiyana?.showToast(err.message, 'danger');
...
async function unpayVisit(visit, onUnpaid) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('housekeeping.visitUnpaidToast'), 'success');
    await onUnpaid();
    refocusAfterRender();
  } catch (err) {
    window.aashiyana?.showToast(err.message, 'danger');
  ...
  content.querySelectorAll('[data-delete-visit]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        window.aashiyana?.showToast(t('housekeeping.visitDeletedToast'), 'success');
        await loadData();
        await loadStaffVisits();
        renderStaff(content);
        refocusAfterRender();
    ...
    onSave: (panel) => {
      panel.querySelector('#housekeeping-task-edit-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('housekeeping.taskUpdatedToast'), 'success');
          await loadData();
          closeModal({ force: true });
          renderTasks(content);
        } catch (err) {
  ...
  openModal({
    ...
    content: `
      <form id="housekeeping-visit-form" class="housekeeping-worker-form">
        <label class="housekeeping-field">
          ...
          <aashiyana-datepicker name="date" type="date" value="${esc(visit.check_in.slice(0, 10))}"></aashiyana-datepicker>
        </label>
    ...
    onSave: (panel) => {
      panel.querySelector('#housekeeping-visit-form')?.addEventListener('submit', async (event) => {
        ...
        if (offGrid) {
          window.aashiyana?.showToast(t('common.amountPrecisionRequired', {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('housekeeping.visitSavedToast'), 'success');
          await loadData();
      ...
      <form id="housekeeping-worker-form" class="housekeeping-worker-form">
        ...
        <div class="housekeeping-form-grid housekeeping-form-grid--wide">
          ...
          <label class="housekeeping-field">
            ...
            <aashiyana-datepicker name="birth_date" type="date" value="${esc(item.birth_date || '')}"></aashiyana-datepicker>
          </label>
          <label class="housekeeping-field">
    ...
    onSave: (panel) => {
      panel.querySelector('#housekeeping-worker-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('housekeeping.workerSavedToast'), 'success');
          await loadData();
          closeModal({ force: true });
...
function describeDeepLinkError(err) {
  ...
  const message = (status >= 400 && status < 500 && status !== 403 && status !== 404)
    ...
    : (window.aashiyana?.friendlyError?.(err) ?? t('common.errorGeneric'));
...
async function openVisitFromDeepLink(editVisitId, container, signal) {
  try {
    ...
    if (offerRetry) {
      window.aashiyana?.showToast(message, 'danger', 6000, {
      ...
      window.aashiyana?.showToast(message, 'danger');
    }
  }
}
