    ...
    onSave(panel) {
      ...
      panel.querySelector('#wtm-archive')?.addEventListener('click', async () => {
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(type.archived ? t('waste.typeRestoredToast') : t('waste.typeArchivedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelector('#wtm-delete')?.addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.typeDeletedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.status === 409 ? t('waste.typeDeleteRefused') : (err.data?.error ?? t('common.unknownError')), 'danger');
        }
      });
      ...
      panel.querySelector('#wtm-save').addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.typeSavedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
...
function openScheduleModal(type, schedule = null) {
  ...
  const content = `
    ...
    <div class="form-group">
      ...
      <aashiyana-datepicker required id="wsm-anchor" name="anchor_date" type="date" value="${esc(schedule?.anchor_date ?? todayKey())}"></aashiyana-datepicker>
    </div>
    <div class="form-group">
      <label class="form-label" for="wsm-valid-until">${t('waste.validUntilLabel')}</label>
      <aashiyana-datepicker id="wsm-valid-until" name="valid_until" type="date" value="${esc(schedule?.valid_until ?? '')}"></aashiyana-datepicker>
    </div>
    <div class="form-group">
      <label class="form-check">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#wsm-delete')?.addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.scheduleDeletedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelector('#wsm-save').addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.scheduleSavedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
...
function openPickupModal(pickup = null) {
  ...
  const content = `
    ...
    <div class="form-group">
      ...
      <aashiyana-datepicker required id="wpm-date" name="date" type="date" value="${esc(pickup?.date ?? todayKey())}"></aashiyana-datepicker>
    </div>
    <div class="form-group">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#wpm-delete')?.addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.pickupDeletedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelector('#wpm-save').addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.pickupSavedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.status === 409 ? t('waste.pickupDuplicateError') : (err.data?.error ?? t('common.unknownError')), 'danger');
        }
      });
    },
  });
    ...
    onSave(panel) {
      ...
      panel.querySelector('#wiz-preview').addEventListener('click', async () => {
        ...
        try {
          ...
          if (res.data.unchanged) {
            closeModal({ force: true });
            window.aashiyana?.showToast(t('waste.sourceUnchangedToast'), 'default');
            return;
          }
      ...
      panel.querySelector('#wiz-commit').addEventListener('click', async () => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('waste.importCommittedToast', { added, changed, removed }), 'success');
        } catch (err) {
          showError(err.data?.error ?? t('common.unknownError'));
        }
      });
...
function openMappingProfileImportModal(source) {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('waste.mappingProfileInvalidFile'), 'danger');
      return;
    }
      ...
      onSave(panel) {
        ...
        panel.querySelector('#wmp-apply')?.addEventListener('click', async () => {
          ...
          try {
            ...
            refocusAfterRender();
            window.aashiyana?.showToast(t('waste.mappingProfileAppliedToast'), 'success');
          } catch (err) {
            stopLoading();
            window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
          }
    ...
    onSave(panel) {
      ...
      panel.querySelector('#wsrc-delete').addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.sourceDeletedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
      ...
      panel.querySelector('#wsrc-save').addEventListener('click', async () => {
        ...
        try {
          ...
          await reloadAndRender();
          refocusAfterRender();
          window.aashiyana?.showToast(t('waste.sourceSavedToast'), 'success');
        } catch (err) {
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
      });
...
async function triggerManualRefresh(id) {
  try {
    ...
    if (res.data.outcome === 'committed') {
      ...
      window.aashiyana?.showToast(t('waste.sourceNeedsMappingToast'), 'default');
    } else if (res.data.outcome === 'error') {
      window.aashiyana?.showToast(res.data.source.last_error ?? t('common.unknownError'), 'danger');
    } else {
      window.aashiyana?.showToast(t('waste.sourceUnchangedToast'), 'default');
    }
  } catch (err) {
    ...
    onSave(panel) {
      ...
      panel.querySelector('#rem-save')?.addEventListener('click', async () => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('waste.reminderSettingsSavedToast'), 'success');
        } catch (err) {
          stopLoading();
          window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
        }
...
async function moveOccurrence(occurrenceKey) {
  ...
  const content = `
    <div class="form-group">
      ...
      <aashiyana-datepicker required id="wmm-date" name="date" type="date" value="${esc(found.occ.date_key)}"></aashiyana-datepicker>
    </div>
    <div class="modal-panel__footer modal-panel__footer--plain">
      <div></div>
      <div style="display:flex;gap:var(--space-3)">
...
function bindEvents() {
  ...
  findPageFab('waste-fab-new-pickup').addEventListener('click', () => {
    ...
    if (!state.types.filter((t2) => !t2.archived).length) {
      window.aashiyana?.showToast(t('waste.addTypeFirstHint'), 'default');
      openTypeModal();
      return;
    }
    openPickupModal();
