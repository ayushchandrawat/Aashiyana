...
let state = {
  ...
  view: localStorage.getItem('aashiyana-documents-view')
    || (typeof matchMedia !== 'undefined' && matchMedia('(max-width: 639px)').matches ? 'list' : 'grid'),
  sort: SORTS.includes(localStorage.getItem('aashiyana-documents-sort'))
    ? localStorage.getItem('aashiyana-documents-sort')
    : 'updated',
  status: 'active',
  category: '',
  folderId: '',
...
function readExpandedFolders() {
  try {
    const raw = JSON.parse(localStorage.getItem('aashiyana-documents-expanded') || '[]');
...
function persistExpandedFolders() {
  try {
    localStorage.setItem('aashiyana-documents-expanded', JSON.stringify([...state.expanded]));
  } catch {
...
function bindPageEvents() {
  ...
  _container.querySelector('#documents-sort')?.addEventListener('change', (e) => {
    ...
    localStorage.setItem('aashiyana-documents-sort', state.sort);
    applyFilters();
    renderDocuments();
  });
  ...
  _container.querySelector('.documents-view-toggle')?.addEventListener('click', (e) => {
    ...
    localStorage.setItem('aashiyana-documents-view', state.view);
...
async function moveFolder(folder) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('documents.folderMovedToast'), 'success');
...
async function renameFolder(folder) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('documents.folderRenamedToast'), 'success');
...
async function deleteFolder(folder) {
  ...
  try {
    ...
    window.aashiyana?.showToast(friendlyError(err), 'danger');
    return;
  }
...
async function applyFolderDeleteResult(
  ...
  if (result.folder_deleted === false && result.contents_changed && hasNonConcurrencyFailure) {
    window.aashiyana?.showToast(t('documents.folderDeleteContentsChangedWithFailuresToast'), 'warning');
  } else if (result.folder_deleted === false && result.contents_changed) {
    window.aashiyana?.showToast(t('documents.folderDeleteContentsChangedToast', {
      deleted: result.deleted_documents,
    }), 'warning');
  } else if (result.folder_deleted === false) {
    window.aashiyana?.showToast(t('documents.folderDeletePartialToast', {
      deleted: result.deleted_documents,
      failed: result.failed_documents?.length || 0,
    }), 'warning');
  } else if (showSuccess && choice === 'delete') {
    window.aashiyana?.showToast(t('documents.folderDeletedWithDocumentsToast', { count: result.deleted_documents }), 'default');
  } else if (showSuccess) {
    window.aashiyana?.showToast(t('documents.folderDeletedToast'), 'default');
  }
...
async function handleFolderDeleteError(err, folder, { delayed = false } = {}) {
  await handleFolderDeleteFailure({
    ...
    delayed,
    translate: t,
    showToast: (...args) => window.aashiyana?.showToast(...args),
...
async function runDocumentAction(action, doc) {
  ...
  if (action === 'archive') {
    ...
    window.aashiyana?.showToast(doc.status === 'archived' ? t('documents.restoredToast') : t('documents.archivedToast'), 'success');
    await loadDocuments();
    renderAll();
  }
  if (action === 'push-dms') {
    ...
    try {
      ...
      window.aashiyana?.showToast(t('documents.pushToDmsQueued'), 'success');
    } catch (err) {
      window.aashiyana?.showToast(err.data?.error ?? t('common.unknownError'), 'danger');
    }
    return;
...
async function moveSelected() {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('documents.bulkMovedToast', { count: docs.length }), 'success');
    await loadDocuments();
    renderAll();
    refocusAfterRender();
  } catch (err) {
...
async function saveFolderUpload(panel, payload) {
  ...
  window.aashiyana?.showToast(toast, outcome.tone);
  return result;
}
...
async function saveDocument(event, doc, panel) {
  ...
  try {
    ...
    if (doc) {
      ...
      window.aashiyana?.showToast(t('documents.savedToast'), 'success');
    } else {
    ...
    onSave(panel) {
      panel.querySelector('#document-folder-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('documents.folderCreatedToast'), 'success');
  ...
  function prepareShare(panel) {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!navigator.canShare({ files: [shareFile] })) {
        window.aashiyana?.showToast(t('documents.shareFailed'), 'danger');
        return;
      }
