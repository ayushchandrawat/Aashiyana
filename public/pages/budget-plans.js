...
function openAddPlan() {
  ...
  if (!options.length) {
    ...
    window.aashiyana?.showToast(t('budget.planAllCategoriesBudgeted'), 'default');
    return;
  }
...
async function savePlan(panel, category, original = null) {
  ...
  try {
    ...
    await load();
    refocusAfterRender();
    window.aashiyana?.showToast(t('budget.planSavedToast'), 'success');
  } catch (err) {
    console.error('[Budget] plan save error:', err);
    btn.disabled = false;
    window.aashiyana?.showToast(t('budget.loadError'), 'danger');
  }
}
...
async function deletePlan(category) {
  ...
  try {
    ...
    await load();
    refocusAfterRender();
    window.aashiyana?.showToast(t('budget.planRemovedToast'), 'default', 5000, async () => {
