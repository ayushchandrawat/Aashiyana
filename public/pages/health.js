      ...
      <form id="vital-form" class="form-stack">
        ...
        <div class="modal-grid modal-grid--2">
          <div class="form-field">
            ...
            <aashiyana-datepicker id="vital-measured-at" type="datetime" value="${esc(localDateTimeValue(now))}"></aashiyana-datepicker>
          </div>
          <div class="form-field">
    ...
    onSave(panel) {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          window.aashiyana?.showToast(t('health.vitals.saved'), 'success');
          ...
          submitBtn.disabled = false;
          window.aashiyana?.showToast(err?.data?.error || t('health.vitals.saveError'), 'danger');
        }
      });
    },
  });
...
// Bedarfsmedikament hat definitionsgemaess keinen Zeitplan. Aashiyana versprach
...
async function handlePrnDose(btn) {
  ...
  try {
    ...
    window.aashiyana?.showToast(err?.data?.error || t('health.meds.doseError'), 'danger');
    return;
  }
  ...
  if (dose != null && Number.isFinite(dose) && med.stock_qty != null) {
    try {
      ...
      window.aashiyana?.showToast(t('health.meds.stockUpdateFailed'), 'danger');
    }
  }

  window.aashiyana?.showToast(t('health.meds.doseSaved'), 'success');
      ...
      <form id="medlog-form" class="form-stack">
        ...
        <div class="modal-grid modal-grid--2" id="medlog-when">
          <div class="form-field">
            ...
            <aashiyana-datepicker id="medlog-date" type="date" value="${esc(dateValue)}"></aashiyana-datepicker>
          </div>
          <div class="form-field">
            <label class="label" for="medlog-time">${esc(t('health.meds.log.timeLabel'))}</label>
            <aashiyana-datepicker id="medlog-time" type="time" value="${esc(timeValue)}"></aashiyana-datepicker>
          </div>
        </div>
    ...
    onSave(panel) {
      ...
      panel.querySelector('#medlog-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('health.meds.log.saved'), 'success');
          await reloadMedViews();
          refocusAfterRender();
        } catch (err) {
          console.error('[Health] med log save error:', err);
          submitBtn.disabled = false;
          window.aashiyana?.showToast(err?.data?.error || t('health.meds.log.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteMedLog(entry) {
  ...
  try {
    ...
    closeModal({ force: true });
    window.aashiyana?.showToast(t('health.meds.log.deleted'), 'success');
    await reloadMedViews();
    refocusAfterRender();
  } catch (err) {
    ...
    onSave(panel) {
      ...
      panel.querySelector('#med-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('health.meds.saved'), 'success');
          await reloadMedViews();
          refocusAfterRender();
        } catch (err) {
          console.error('[Health] med save error:', err);
          submitBtn.disabled = false;
          window.aashiyana?.showToast(err?.data?.error || t('health.meds.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteMed(med) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('health.meds.deleted'), 'success');
    await reloadMedViews();
    refocusAfterRender();
  } catch (err) {
    console.error('[Health] med delete error:', err);
    window.aashiyana?.showToast(err?.data?.error || t('health.meds.deleteError'), 'danger');
  }
}
    ...
    <div class="health-sched-add">
      <div class="modal-grid modal-grid--2">
        <div class="form-field">
          ...
          <aashiyana-datepicker id="sched-time" type="time" value="08:00"></aashiyana-datepicker>
        </div>
        <div class="form-field">
      ...
      <form id="lab-form" class="form-stack">
        <div class="modal-grid modal-grid--2">
          <div class="form-field">
            ...
            <aashiyana-datepicker id="lab-date" type="date" value="${esc(dateValue)}"></aashiyana-datepicker>
          </div>
          <div class="form-field">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#lab-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('health.labs.saved'), 'success');
          await reloadLabs();
          refocusAfterRender();
        } catch (err) {
          console.error('[Health] lab save error:', err);
          submitBtn.disabled = false;
          window.aashiyana?.showToast(err?.data?.error || t('health.labs.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteLabReport(report) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('health.labs.deleted'), 'success');
    if (labs.selectedReportId === report.id) labs.selectedReportId = null;
    await reloadLabs();
    refocusAfterRender();
  } catch (err) {
    console.error('[Health] lab delete error:', err);
    window.aashiyana?.showToast(err?.data?.error || t('health.labs.deleteError'), 'danger');
  }
}
  ...
  openModal({
    ...
    content: `
      <form id="activity-form" class="form-stack">
        ...
        <div class="form-field">
          ...
          <aashiyana-datepicker id="activity-performed-at" type="datetime" value="${esc(dateValue)}"></aashiyana-datepicker>
        </div>
        <div class="modal-grid modal-grid--3">
          <div class="form-field">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#activity-form').addEventListener('submit', async (e) => {
        ...
        if (!body) {
          window.aashiyana?.showToast(t('health.activity.invalid'), 'danger');
          return;
        }
        submitBtn.disabled = true;
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('health.activity.saved'), 'success');
          ...
          submitBtn.disabled = false;
          window.aashiyana?.showToast(err?.data?.error || t('health.activity.saveError'), 'danger');
        }
      });
    },
  });
...
async function deleteActivity(row) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('health.activity.deleted'), 'success');
    await reloadActivity();
    refocusAfterRender();
  } catch (err) {
    console.error('[Health] activity delete error:', err);
    window.aashiyana?.showToast(err?.data?.error || t('health.activity.deleteError'), 'danger');
  }
}
    ...
    <div class="health-overview__export">
      <div class="health-overview__export-range">
        <div class="form-field">
          ...
          <aashiyana-datepicker id="ov-export-from" type="date" value="${esc(from || '')}"></aashiyana-datepicker>
        </div>
        <div class="form-field">
          <label class="label" for="ov-export-to">${esc(t('health.export.rangeTo'))}</label>
          <aashiyana-datepicker id="ov-export-to" type="date" value="${esc(to || '')}"></aashiyana-datepicker>
        </div>
      </div>
...
function wireOverview() {
  ...
  overview.root.querySelectorAll('[data-vital-nav]').forEach((card) =>
    card.addEventListener('click', () => {
      ...
      window.aashiyana?.navigate('/health/vitals');
    }));
  ...
  overview.root.querySelector('[data-action="ov-go-meds"]')
    ?.addEventListener('click', () => window.aashiyana?.navigate('/health/meds'));
...
async function cycleStartPeriodToday() {
  ...
  try {
    ...
    cycle.anchor = today;
    window.aashiyana?.showToast(t('health.cycle.today.startedToast'), 'success');
    await reloadCycle();
  } catch (err) {
    console.error('[Health] cycle start error:', err);
    window.aashiyana?.showToast(err?.data?.error || t('health.cycle.saveError'), 'danger');
  }
}

async function cycleEndPeriodToday() {
  const open = cycleOpenPeriod();
  if (!open) { window.aashiyana?.showToast(t('health.cycle.today.noOpenPeriod'), 'info'); return; }
  try {
    await api.patch(`/health/cycle/periods/${open.id}`, { end_date: todayKey() });
    window.aashiyana?.showToast(t('health.cycle.today.endedToast'), 'success');
    await reloadCycle();
  } catch (err) {
  ...
  openModal({
      ...
      <form id="cycle-period-form" class="form-stack">
        <div class="modal-grid modal-grid--2">
          <div class="form-field">
            ...
            <aashiyana-datepicker id="cycle-start" type="date" value="${esc(startVal)}"></aashiyana-datepicker>
          </div>
          <div class="form-field">
            <label class="label" for="cycle-end">${esc(t('health.cycle.field.endDate'))}</label>
            <aashiyana-datepicker id="cycle-end" type="date" value="${esc(endVal)}"></aashiyana-datepicker>
          </div>
        </div>
        <div class="form-field">
    ...
    onSave(panel) {
      ...
      panel.querySelector('#cycle-period-form').addEventListener('submit', async (e) => {
        ...
        if (!start) { window.aashiyana?.showToast(t('health.cycle.invalid'), 'danger'); return; }
        if (end && end < start) { window.aashiyana?.showToast(t('health.cycle.invalid'), 'danger'); return; }
        const body = {
          start_date: start,
          end_date: end || null,
        ...
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('health.cycle.saved'), 'success');
          await reloadCycle();
          refocusAfterRender();
        } catch (err) {
...
async function deletePeriod(period) {
  ...
  try {
    ...
    window.aashiyana?.showToast(t('health.cycle.deleted'), 'success');
    await reloadCycle();
    refocusAfterRender();
  } catch (err) {
    console.error('[Health] cycle period delete error:', err);
    window.aashiyana?.showToast(err?.data?.error || t('health.cycle.deleteError'), 'danger');
  }
}
    ...
    content: `
      <form id="cycle-settings-form" class="form-stack">
        ...
        <div class="form-field" id="cs-due-field" ${s.pregnancy_mode ? '' : 'hidden'}>
          ...
          <aashiyana-datepicker id="cs-due" type="date" value="${esc(s.pregnancy_due_date || '')}" min="${esc(dueMin)}" max="${esc(dueMax)}"></aashiyana-datepicker>
        </div>
    ...
    onSave(panel) {
      ...
      bulkRun?.addEventListener('click', async () => {
        ...
        try {
          ...
          showBulk(false);
          window.aashiyana?.showToast(t('health.cycle.settings.applyToAllDone', { count }), 'success');
          await reloadCycle();
        } catch (err) {
      ...
      panel.querySelector('#cycle-settings-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          closeModal({ force: true });
          window.aashiyana?.showToast(t('health.cycle.settings.saved'), 'success');
          renderCycleShell();
        } catch (err) {
