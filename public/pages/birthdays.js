...
function bindEvents() {
  ...
  try {
    if (sessionStorage.getItem('aashiyana:birthdays:autoImport')) {
      sessionStorage.removeItem('aashiyana:birthdays:autoImport');
      openImportModal();
    }
  ...
  openSharedModal({
        ...
        <div class="birthday-modal__identity">
          ...
          <div class="birthday-modal__fields">
            ...
            <div class="form-group">
              ...
              <aashiyana-datepicker id="bd-birth-date" type="date" max="${today}" value="${esc(birthday?.birth_date || '')}"></aashiyana-datepicker>
            </div>
          </div>
        </div>
        ${advancedSection(`
    ...
    onSave(panel) {
      ...
      panel.querySelector('#bd-save').addEventListener('click', async () => {
        ...
        if (!nameDay.complete) {
          window.aashiyana?.showToast(t('birthdays.nameDayIncomplete'), 'warning');
          return;
        }
        const body = {
        ...
        if (!body.name || !body.birth_date || !isDateInputValid(birthDateRaw)) {
          window.aashiyana?.showToast(t('birthdays.requiredFields'), 'warning');
          return;
        }
        ...
        try {
          if (isEdit) {
            ...
            window.aashiyana?.showToast(t('birthdays.updatedToast'), 'success');
          } else {
            await api.post('/birthdays', body);
            window.aashiyana?.showToast(t('birthdays.createdToast'), 'success');
          }
          await loadData();
          renderList();
  ...
  openSharedModal({
    ...
    onSave(panel) {
      ...
      submitBtn.addEventListener('click', async () => {
        ...
        if (ids.length === 0) {
          window.aashiyana?.showToast(t('birthdays.importNothingSelected'), 'warning');
          return;
        }
        submitBtn.disabled = true;
        try {
          const res = await api.post('/birthdays/import', { contact_ids: ids });
          window.aashiyana?.showToast(t('birthdays.importSuccess', { count: res.data.imported }), 'success');
          await loadData();
          renderList();
          closeModal({ force: true });
        } catch (err) {
