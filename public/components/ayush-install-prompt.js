...
const SEEN_KEY = 'aashiyana-install-seen';
...
class AashiyanaInstallPrompt extends HTMLElement {
  constructor() {
    super();
  ...
  async _onInstallClick() {
    try {
      ...
      console.log('[aashiyana-install-prompt] Ergebnis:', result.outcome);

      if (result.outcome === 'accepted') {
        this._remove();
      }
    } catch (err) {
      console.error('[aashiyana-install-prompt] Fehler:', err);
    }
    this._deferredPrompt = null;
  }
...
customElements.define('aashiyana-install-prompt', AashiyanaInstallPrompt);
