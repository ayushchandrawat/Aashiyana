#!/usr/bin/env node
/**
 * Aashiyana Web Installer - temporary setup server.
...
export function renderEnvFile(env) {
  const lines = ['# Generated by Aashiyana Installer'];
...
async function route(req, res, server) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/preflight') {
    try {
      ...
      const { cmd, args } = inspectCommand(engine, ['inspect', '--format', '{{.State.Status}}', 'aashiyana']);
  ...
  if (req.method === 'GET' && url.pathname === '/api/status') {
    ...
    const health = inspectCommand(engine, ['inspect', '--format', '{{.State.Health.Status}}', 'aashiyana']);
    const stateCmd = inspectCommand(engine, ['inspect', '--format', '{{.State.Status}}', 'aashiyana']);
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        const proxyReq = http.request(opts, proxyRes => {
          ...
          proxyRes.on('end', () => {
            ...
            catch { resolve({ status: proxyRes.statusCode, body: { error: 'Invalid response from Aashiyana' } }); }
          });
        });
