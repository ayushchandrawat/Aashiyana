...
- **Pull request [#2847](https://github.com/awesome-selfhosted/awesome-selfhosted-data/pull/2847)** - adds `software/aashiyana.yml`, CI green (`make awesome_lint`, hecat 1.6.0). This is the path their CONTRIBUTING prefers; the issue is only the fallback for people who would rather not send a PR.
...
`aashiyana.yml` next to this file is the byte-exact content of the submitted entry. Their linter enforces the rules that matter, so keep both in step:
...
cp docs/awesome-selfhosted/aashiyana.yml awesome-selfhosted-data/software/
