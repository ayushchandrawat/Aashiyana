...
Aashiyana can replace the dashboard with photos from an [Immich](https://immich.app/) server after
five minutes without keyboard, pointer, touch, or scroll activity. It requests a random set of
photos, changes the photo every 20 seconds, and returns to Aashiyana on the next interaction. Photo
metadata moves between corners so that the screensaver does not introduce another fixed bright
area. If Immich is unavailable, Aashiyana leaves the current screen visible and tries again after a
later idle period.
...
The Immich server must be reachable from the Aashiyana server or container. A URL that works only in
...
Aashiyana's server can reach Immich.

## Configure in Aashiyana
...
and their matching controls become read-only in Settings. Restart Aashiyana after changing them.

## Security and troubleshooting

- Enable `DB_ENCRYPTION_KEY` to encrypt a key saved through Settings at rest. Without database
  encryption, Aashiyana logs a warning when it stores an Immich API key.
- Aashiyana proxies thumbnails so the API key is never placed in image URLs or sent to the tablet.
