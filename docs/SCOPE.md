# What Aashiyana will not become

Aashiyana is maintained by one person, and every integration in the core is a promise without
...
- **Not this way.** The goal is right, the proposed route is not. There is usually another
  route on this page.
- **Not here.** It belongs next to Aashiyana rather than inside it - as a third-party module
...
"Not at all" above declines, or a membership model that expects strangers. It is not that Aashiyana
...
**Aashiyana does not connect to your bank, and does not adopt another finance app's data
...
assume **one company running one service for many users**, and Aashiyana is the opposite
...
than Aashiyana's Budget module does, and sitting next to it is sensible. But Actual has no
ordinary HTTP API. Integrations go through `@actual-app/api`, a Node package that opens the
**local** budget file and runs Actual's own sync engine alongside Aashiyana. That means a
...
- **A mapping is saved and shareable - Aashiyana ships no bank list.** This is about
  maintenance, not comfort. If Aashiyana ships bank profiles, every missing bank is an issue
  against Aashiyana and that queue never ends. If a mapping is something you save after your
  first import and can export, a community repository is people helping each other and
  Aashiyana stays out of it. Same benefit, none of the ownership.
...
- **The acceptance test needs no bank:** Aashiyana must read back what
...
Aashiyana image - encrypted token storage, a scheduler and a bank API, talking to Aashiyana
...
**Aashiyana does not take on a permanent binding to somebody else's cloud in the core.** Every
...
### The shape that works: Aashiyana stays the server, the bridge is a client
...
*unmodified* Aashiyana image - encrypted token storage, a scheduler and a bank API, all through
`/api/v1` and the `modules/` directory, with no core patch and no second writer on
`aashiyana.db` (#746). Since v2.63.0 a third-party module also has the same surfaces a core
...
- **A product database with nutrition and package sizes** (#714) is declined, and the
  ...
  is a fact about a *product*, and manufacturers change recipes and package sizes. Aashiyana
...
The rule is often stated as "Aashiyana has no dependencies". That is not true, and stating it
...
the backend does. Writing every specialist domain in-house would make Aashiyana the maintainer
...
1. **Would writing it ourselves make Aashiyana the maintainer of a specialist domain?** If
...
- **No translation platform.** The 24 locales live in the repository as JSON, and a guard
  ...
  writes.
- **No hosted demo.** Aashiyana's promise is that a household's data stays on the household's
  ...
  ```bash
  node scripts/seed-demo.js --db ./aashiyana.db --locale en
  ```
