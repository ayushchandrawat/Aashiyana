# Aashiyana - Product Specification
...
| code_hash | TEXT | NOT NULL. SHA-256, deliberately not bcrypt: the code is not a password but a ~49-bit random value from Aashiyana's own generator. Key stretching protects weak secrets; here there is nothing weak to protect, while ten bcrypt runs per sign-in attempt would make the sign-in itself the target |
...
The flag is copied onto the follow-up instance; without that the series would fall back to the due-date grid from its second run on, and it would do so silently, because the follow-up looks complete either way. The completion day is read in the household's own zone (`TZ`, see `server/utils/timezone.js`): ticking a task off at 00:30 must count as the new day, or a weekly task would come back six days later. The anchor is local to Aashiyana and does not travel over CalDAV, because RFC 5545 has no way to express it and a mirrored VTODO carries the rule alone. The shared calculation lives in `nextDueAfterCompletion()` in `server/services/recurrence.js`, deliberately separate from the route because resettable countdowns want the same "counts from the moment you touched it" arithmetic (#647).
...
**Category default repaired (migration v114,
...
- **UI:** tags are chips on the task card and the Kanban card (three, then a `+N` summary); clicking one filters by it. A tag editor sits in the task dialog, a "Manage tags" modal (`aashiyana-tag-manager`) in the toolbar, and add/remove tag actions in the bulk bar.
- **CalDAV:** mirrored from `CATEGORIES` on VTODO objects and pushed back. `CATEGORIES` is only in the managed property set because Aashiyana holds the **complete** list — mirroring a single value would make every push delete the tags the server knows and Aashiyana never saw.
...
| countdown | INTEGER | NOT NULL DEFAULT 0 (migration v150,
...
**Editing and deleting occurrences of a recurring series (migration v85 ·
...
longer returns, so removing an event in iCloud/Nextcloud also removes it in Aashiyana. Deletion is scoped
...
automatically into that one calendar — the recommended setup is a dedicated "Aashiyana" calendar in
...
| outlook_change_key | TEXT | Graph ETag after Aashiyana's last own write, nullable — the basis of drift reconciliation |
...
back to the Aashiyana state; an event missing from the listing was deleted in Outlook and is
...
makes "Aashiyana is the source of truth" hold against remote edits at every interval, at the cost of
roughly one extra request per calendar per run.

### Calendar Pending Deletions
Tombstones for events deleted in Aashiyana that still exist at the provider (migration v103, #593).
...
- A change is a **patch of the original object**, never a rebuild: only the mirrored properties
  ...
  visibility and subtasks are Aashiyana-internal and never trigger a push.
...
- **Creating: outbound since migration v136 (#695).** Until then this was out of scope, on the
  ...
  directions" the whole time. A task created in Aashiyana now names its destination in
  ...
  - The UID is derived from the task id (`aashiyana-task-<id>@aashiyana.local`), not drawn at random: a
...
- **Lossy mappings are held from both ends.** Aashiyana has four priority levels and three statuses,
...
Aashiyana now keeps the structured `N` components (family, given, additional, prefix, suffix) and
...
DB-backed, customizable category list for contacts. Replaces the old hardcoded German-named set. The seven predefined keys (`doctor`, `school`, `authority`, `insurance`, `craftsman`, `emergency`, `misc`) carry a stable slug key (which, together with `icon`, drives the list grouping), a localizing `label_key`, a Lucide `icon`, and a `color`; the pre-existing German category values (`Arzt`, `Behörde`, …) are migrated to these keys. User-added categories store their `name` and default to the `tag` icon. A "Manage categories" button in the contacts toolbar opens the shared `aashiyana-category-manager` modal to add, rename, recolor, reorder, and delete categories, with the same in-use / last-category deletion guards as Tasks and Budget.
...
**Booking only after confirmation (migration v129,
...
**Webhook payload template.** An empty template sends Aashiyana's own body shape, which suits receivers
...
**Own currency per loan (migration v102,
...
into the budget because it is being paid now, whereas these were paid before Aashiyana existed and never
...
`localStorage` under `aashiyana:calendar:waste:visibleTypes`, following the same "empty set means
...
`DB_ENCRYPTION_KEY` (SQLCipher) is strongly recommended. Aashiyana is **not** a medical device and
...
**ICS export:** the UID scheme widens for extras only (`schedule-entry-{userId}-{dateKey}@aashiyana`
...
**The header band can be switched off (v2.14.0 ·
...
**Views:**
- List view (default): grouped by category or due date (toggleable), filter: person, priority, status. **Category groups follow the managed order (v2.39.0,
- **Collapsible groups (v2.28.0,
...
- **Manual item order within an aisle (v1.87.0,
- **Send the list to a member by email (#944):** an entry in the overflow menu mails the list's open
  ...
  data in Aashiyana, and a leaked link stays leaked; someone who needs the list continuously is a member
...
- **Flexible time entry (Discussion
...
- **New events follow the displayed period (v2.10.1 ·
...
- **"Unassigned" on the person axis (#1064):** the person list gains a row for the events and tasks nobody is assigned to, stored as the value `none` in the same `aashiyana:calendar:people` list (a string, so it cannot collide with a user id). On its own it shows only the unassigned entries; together with people it adds them. A person filter saved before this row existed does not contain it, so unassigned events stay out under it, as they did (#987).
- **Calendar and subscription filter (#1064):** the filter sheet lists every connected calendar and ICS subscription with events in the loaded range, each with its colour and a switch; hiding one removes its events from all four views and the agenda. It is a filter, not the colour legend the person filter above deliberately avoids: an event's colour still resolves through `resolveEventColor()`. The hidden set is stored on the device **per account** (`aashiyana:calendar:sources-hidden:<userId>`) together with each source's name and colour, so a hidden source stays listed and can be switched back on even when none of its events is on screen, and a private subscription's name does not show to the next account on a shared browser. A new event created for a hidden calendar stays out before it has synced, and an event with a pending move counts under its destination calendar.
...
- **Birthday layer (v2.28.0,
- **File attachments:** Events support a single file attachment (images, PDFs, Office documents, up to `MAX_UPLOAD_MB`, default 5 MB — the shared upload ceiling since v2.28.0,
...
- **Region / Format presets (Settings → Personal → Appearance):** a household-wide **Region** selector (admin-only) sets currency, **number**, date and time format together from one BCP-47 region (e.g. Switzerland → `1'234.50`). Number and currency grouping follow the selected region independently of the UI language (v1.29.0) — a German-language household can still display Swiss-formatted amounts — resolved via a `getFormatLocale()` that reads the stored region and falls back to the UI language; choosing **Custom** configures each format individually. Non-admin members see a read-only notice. Budget CSV export uses a dot decimal without grouping so a comma-decimal locale never collides with the comma field delimiter.
  ...
  **69 regions.** `public/settings/region-presets.js` covers Europe, Asia, Oceania and — since v1.78.0 — every sovereign state of the Americas including the Caribbean, plus the Philippines (`en-PH`, `fil-PH`). **Every shipped language has had at least one region since
...
- **Household time zone (Settings → Personal → Appearance → Region, admin-only, v2.34.0 ·
  ...
  **The bug this surfaced (v2.34.0 ·

  **One currency list, one place (v1.61.0, unified in v2.35.0 ·
...
- Reminder time set via the shared `aashiyana-datepicker` in the task or event modal, usually as an offset from the due date/start
...
| `manifestVersion` | | Integer format number of the manifest itself, not of the module. Omitted means 1; a higher number than this Aashiyana reads is rejected outright, never read in part. |
...
- Prefer `/api.js` for Aashiyana REST calls. It prefixes `/api/v1`, sends session credentials, refreshes CSRF tokens, and uses non-cached fetches for user data.
...
- **Toast gesture (`wireSwipeToDismiss` in `public/utils/ux.js`,
...
- **Read-only offline data (v0.78.8):** The service worker network-first-caches a whitelist of read-only `GET /api/v1/*` data paths (calendar, tasks, shopping, contacts, dashboard) in a release-keyed `aashiyana-api-<version>` cache, so the last-seen data stays viewable offline. Mutations, `/auth/*`, and non-whitelisted GETs are never cached; state-changing requests that fail offline surface a clear "changes aren't possible while offline" message instead of a raw network error. The calendar shows a subtle "Offline – as of: {time}" banner (from the cached `x-cached-at` timestamp) when served from cache. The API cache is wiped on logout and session expiry (`CLEAR_API_CACHE` message) so a second user on the same device cannot see the previous user's cached data, and every cache that does not belong to the running release is purged on SW activation — previous `aashiyana-*` versions as well as the legacy `oikos-*` caches from before the rename.
...
1. `localStorage` entry `aashiyana-locale` (manual selection)
