...
Three ways to get Aashiyana running from scratch:

### Option A — Web Installer (recommended, all platforms)

```bash
git clone https://github.com/AyushChandrawat/Aashiyana.git && cd aashiyana
...
`docker compose exec aashiyana node setup.js` (or the matching `podman compose … exec`).

---

# Installation Guide

Complete setup instructions for Aashiyana - from Docker installation to your first login.
...
For local-only access, the Docker container is all you need. If you want to access Aashiyana from other devices on your network or the internet, add Nginx as a reverse proxy with SSL.
...
Docker. Aashiyana supports Podman out of the box: both installers auto-detect it, and a
...
There are seven ways to get Aashiyana running. **Option A** (web installer) is recommended for most users — it walks you through every step in your browser. **Option B** (pre-built image) is a quick manual alternative. **Option C** (build from source) is for contributors or custom builds. **Options D–F** install directly from a NAS/home-server app store with no terminal required: **Option D** (TrueNAS SCALE), **Option E** (Umbrel), and **Option F** (Unraid). **Option G** covers Portainer, whether you paste the stack or let it follow this repository via Git.
...
- Advanced settings — Single Sign-On (OIDC), the three home-network permissions (they lift the SSRF protection and are asked as one group), the calendar sync interval, live currency rates and the Web-Push contact. Everything that decides what Aashiyana connects to
...
The final screen also links to the next three steps on your new instance: inviting your family, choosing which modules to enable, and installing Aashiyana on your phones. Running the wizard again on an installation that already has an admin account is a supported case — it writes your `.env`, restarts the container and takes you to that same screen instead of failing.
...
Docker pulls `ghcr.io/AyushChandrawat/aashiyana:latest` automatically. No build step, no Node.js installation needed.
...
> set `image: ghcr.io/AyushChandrawat/aashiyana:2.66.1` in your compose file and bump it
...
>
> ```bash
> cosign verify ghcr.io/AyushChandrawat/aashiyana:2.66.1 \
>   --certificate-oidc-issuer https://token.actions.githubusercontent.com \
>   --certificate-identity-regexp '^https://github.com/AyushChandrawat/Aashiyana/.github/workflows/docker-publish.yml@refs/tags/v'
> ```
>
...
> image: `docker buildx imagetools inspect ghcr.io/AyushChandrawat/aashiyana:2.66.1 --format '{{ json .Provenance }}'`.
...
```
aashiyana  | [Aashiyana] Server running on port 3000 | Version 2.66.1
aashiyana  | [Aashiyana] Environment: production
aashiyana  | [Sync] Auto-sync active every 15 minutes.
```
...
On the first visit, Aashiyana detects that no account exists yet and guides you through
...
After you submit, Aashiyana creates the admin, signs you in automatically, and the setup
...
```bash
docker compose exec aashiyana node setup.js
```
...
No terminal required. Aashiyana is available directly in the TrueNAS SCALE Community Apps Catalog.

#### 1. Open the Apps Catalog

In your TrueNAS SCALE web UI, go to **Apps → Discover Apps** and search for **Aashiyana**.
...
No terminal required. Aashiyana is available in the Umbrel App Store — everything runs on, and stays on, your Umbrel.

#### 1. Open the App Store

In your Umbrel dashboard, open the **App Store** and search for **Aashiyana**.
...
Launch Aashiyana from your Umbrel home screen. The first visit guides you through creating your admin account in the browser.
...
No terminal required. Aashiyana ships as an Unraid Community Applications template.

#### 1. Open Community Applications

In Unraid, open the **Apps** tab (the Community Applications plugin) and search for **Aashiyana**.
...
Click **Apply**. Once the container is running, click the Aashiyana icon → **WebUI**. The first visit guides you through creating your admin account in the browser.
...
**Stacks → Add stack → Repository**, repository `https://github.com/AyushChandrawat/Aashiyana`, branch `main`, and set the **Compose path** to:
...
| `OIKOS_HTTP_PORT` | Host port that the compose file maps to the container's port 3000. Change this to expose Aashiyana on a different host port; the app inside the container always listens on 3000. | `3000` | No |
...
Aashiyana backend container or host must be able to reach the configured base URL. HTTPS is recommended;
...
- **The app must be installed to the Home Screen.** iOS delivers Web Push only to installed
  home-screen web apps, never to a Safari tab. Open Aashiyana in Safari, then Share ->
  "Add to Home Screen".
...
- **Check iOS Settings -> Notifications -> Aashiyana**: "Allow Notifications" must be on, and a Focus
...
- **The VAPID subject must be routable.** Apple validates the contact URI in the signed token and
  ...
  to work. Aashiyana derives a usable value from the SMTP sender address or `BASE_URL`; set
...
open an immediate preview. An optional album UUID limits the selection; otherwise Aashiyana uses the
...
| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `DB_PATH` | Path to the SQLite database file inside the container | `/data/aashiyana.db` | No |
...
| `DB_ALLOW_NEWER_SCHEMA` | Emergency switch, normally unset. An older Aashiyana refuses to start on a database a newer version has opened (see [Going back](#going-back)); `1` starts it anyway, at your own risk, with a warning on every start. | - | No |
...
> ```yaml
> volumes:
>   - /mnt/user/appdata/aashiyana/data:/data
>   - /mnt/array/aashiyana-backups:/backups   # host side is yours to choose
> ```
>
...
Uploads fail closed: if WebDAV cannot accept the file, Aashiyana rejects the upload instead of silently
...
below. Aashiyana requests only `https://www.googleapis.com/auth/drive.file`; it cannot browse arbitrary
...
activate it. New files are placed in the visible private `Aashiyana/Documents` folder; the opaque Drive
...
> **Access and backup boundary:** Aashiyana visibility settings only control access through Aashiyana.
...
> the same LAN or Docker network as Aashiyana, so blocking private targets by default would break
...
events created, edited, deleted, or moved to another calendar in Aashiyana are applied in Google as
...
Aashiyana from changing anything in Google while still importing normally.
...
One-way push **Aashiyana → Outlook.com** for personal Microsoft accounts (outlook.com, hotmail.com, M365 Family). Outlook.com does not support CalDAV, so this provider uses the Microsoft Graph API. Aashiyana stays the source of truth: pushed events are created/updated/deleted in Outlook, and every sync run also checks the pushed events for remote drift (one cheap `changeKey` listing per calendar) — events edited in Outlook are reset to the Aashiyana state, events deleted in Outlook are re-created. Multiple family accounts can be connected.
...
2. Name: e.g. `Aashiyana Calendar Push`. Supported account types: **"Personal Microsoft accounts only"**.
...
Aashiyana are applied on the server as well, and changes made there flow back. An outbound change is
attempted right when you save and retried by the next sync run if the server cannot be reached.
Editing preserves everything the server holds that Aashiyana does not — attendees, alarms, categories
...
Nothing to configure — there is no environment variable, and Aashiyana never reaches the network for
...
**Single sign-on does not skip it.** If you have a second factor set up, Aashiyana asks for the code
...
Time matters: TOTP codes are derived from the clock, and Aashiyana accepts a deviation of ±30 seconds.
...
Pocket ID documents Aashiyana as one of its [client examples](https://pocket-id.org/docs/client-examples/aashiyana), which is a working set of values for the four variables below if you run that provider.

| Variable | Description | Default | Required |
|----------|-------------|---------|----------|
| `OIDC_ISSUER` | OIDC provider issuer URL (e.g. `https://authentik.example.com/application/o/aashiyana/`) | - | No |
...
| `OIDC_REDIRECT_URI` | OAuth callback URL — must be registered with the provider (e.g. `https://aashiyana.example.com/api/v1/auth/oidc/callback`) | - | No |
...
| `WEBDAV_BACKUP_PATH` | Remote directory path for backup files | `/aashiyana/backups/` | No |
...
> **Optional for local access, required for network/internet access.** If you only access Aashiyana on the same machine (localhost), you can skip this section.

When exposing Aashiyana to your local network or the internet, you need HTTPS for security. Nginx acts as a reverse proxy that handles SSL termination and forwards requests to the Docker container.
...
Aashiyana ships with an example configuration. Copy it and replace `deine-domain.de` with
...
```bash
sudo cp nginx.conf.example /etc/nginx/sites-available/aashiyana
sudo nano /etc/nginx/sites-available/aashiyana   # replace deine-domain.de
```
...
`ssl_certificate` lines in `/etc/nginx/sites-available/aashiyana`, then link and reload:

```bash
sudo nano /etc/nginx/sites-available/aashiyana   # uncomment ssl_certificate + ssl_certificate_key
sudo ln -s /etc/nginx/sites-available/aashiyana /etc/nginx/sites-enabled/
...
On RHEL-based systems you can run Aashiyana as a rootless systemd service via Podman
[Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html). Aashiyana
...
> **One-time step when upgrading past the container rename (Oikos → Aashiyana):** the Docker/Podman
> service and container were renamed from `oikos` to `aashiyana`. Your data is safe - the database
> volume is unchanged and the app migrates an existing `oikos.db` to `aashiyana.db` automatically on
> first start. But the old `oikos` container lingers as an orphan and keeps holding host port 3000,
> which prevents the new `aashiyana` container from starting. Run the update once with
> `--remove-orphans`:
>
> ```bash
...
backup from an older Aashiyana restores into any newer one, and a database that a newer version has
...
The SQLite database lives in the host folder configured through `DATA_DIR` and is mounted at `/data` inside the container. The database file is `/data/aashiyana.db`.
...
```bash
docker compose exec aashiyana node -e "import('./server/db.js').then(async db => { await db.backupToFile('/data/aashiyana-backup.db'); process.exit(0); })"
docker cp aashiyana:/data/aashiyana-backup.db ./aashiyana-backup-$(date +%Y%m%d).db
```
...
```bash
SERVICE=aashiyana
BACKUP="$PWD/aashiyana-backup-20260401.db"
docker compose stop "$SERVICE"
docker compose run --rm -v "$BACKUP:/tmp/aashiyana-restore.db:ro" --entrypoint sh "$SERVICE" -c 'set -eu; target="${DB_PATH:-/data/aashiyana.db}"; case "$target" in */oikos.db) target="${target%/oikos.db}/aashiyana.db";; esac; stamp=$(date -u +%Y%m%dT%H%M%SZ); if [ -f "$target" ]; then cp "$target" "$target.pre-restore-$stamp"; fi; rm -f "$target-wal" "$target-shm"; cp /tmp/aashiyana-restore.db "$target"; chown node:node "$target" 2>/dev/null || true'
...
```bash
DB_PATH=/path/to/aashiyana.db node --import dotenv/config scripts/restore-backup.js ./aashiyana-backup-20260401.db
```

The restore helper validates that the file is a Aashiyana database, and refuses one written by a newer Aashiyana than the one running (update first, then restore), before replacing the active database. It also keeps a pre-restore copy next to the database file for emergency rollback.
...
```
0 3 * * * docker compose exec -T aashiyana node -e "import('./server/db.js').then(async db => { await db.backupToFile('/data/aashiyana-cron-backup.db'); process.exit(0); })" && docker cp aashiyana:/data/aashiyana-cron-backup.db /path/to/backups/aashiyana-$(date +\%Y\%m\%d).db
```
...
target" field, and a task created in Aashiyana with a target set is uploaded on the next run (or right
...
to everyone but its creator and admins inside Aashiyana, but an **inbound sync still rewrites its
...
account. If you want your Apple tasks in Aashiyana, keep them in a CalDAV-backed list (Nextcloud,
...
reverse — deleting an event in Aashiyana removes it from Outlook on the *next* run, not instantly.

Editing a pushed event in Outlook is pointless: Aashiyana is the source of truth and resets it to its
own state on the next run, and re-creates it if you delete it there. To get rid of an event for
good, delete it in Aashiyana. Note also that disconnecting an account leaves everything already
pushed behind in Outlook — clear those events in Aashiyana *before* disconnecting, or delete them by
...
Before that, Aashiyana discarded any calendar object whose URL did not contain `.ics`. That extension
