  ...
  return {
    ...
    '/api/v1/documents/storage/google-drive/test': {
      post: op({
        ...
        admin: true,
        stateChanging: true,
        description: 'Runs a temporary create, read, verify and delete roundtrip in the private Aashiyana/Documents folder.',
        responses: {
          200: {
            description: 'Connection roundtrip succeeded',
            content: { 'application/json': { schema: { $ref: '#/components/schemas/DocumentStorageTestResponse' } } },
    ...
    '/api/v1/documents/storage/google-drive/disconnect': {
      delete: op({
        ...
        admin: true,
        stateChanging: true,
        description: 'Deletes only Aashiyana\'s local Drive token state. It does not revoke shared Google credentials. Disconnection is blocked while Drive is selected or Drive-backed documents exist.',
        responses: {
          200: {
