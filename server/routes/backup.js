...
function backupFileName() {
  ...
  return `aashiyana-backup-${stamp}.db`;
}
...
router.get('/database', requireAdmin, async (req, res) => {
  let tmpPath = null;
  try {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'aashiyana-backup-'));
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      dir = await fs.mkdtemp(path.join(os.tmpdir(), 'aashiyana-restore-'));
