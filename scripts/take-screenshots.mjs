...
const DEMO_DB     = '/tmp/aashiyana-screenshot.db';
const PORT        = 3099;
...
async function dismissOverlays(page) {
  await page.evaluate(() => {
    document.querySelectorAll('.onboarding-overlay, aashiyana-install-prompt').forEach((el) => el.remove());
  });
...
async function applyAppState(page, theme, locale) {
  await page.evaluate((a) => {
    localStorage.setItem('aashiyana-locale', a.locale);
    localStorage.setItem('aashiyana-onboarded', '1');
    localStorage.setItem('aashiyana-install-dismissed', String(Date.now()));
    localStorage.setItem('aashiyana-theme', a.theme);
